...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchProducts() {
        try {
            const response = await fetch('https://rees.greygladyreigh.workers.dev/products'); // Adjust the URL as needed
            if (!response.ok) {
            ...
            // Assume products are inside a data property
            ...
            if (Array.isArray(fetchedProducts) && fetchedProducts.length > 0) {
                // Combine fetched and hardcoded products
                const allProducts = fetchedProducts.concat(hardcodedProducts);

                // Sort combined products by price (low-to-high) before rendering
                ...
                // Sort hardcoded products by price (low-to-high) before rendering
                hardcodedProducts.sort((a, b) => a.price - b.price);
                renderProducts(hardcodedProducts); // Render hardcoded products if no fetched products
            }
        } catch (error) {
            console.error('Error fetching products:', error);
            // Sort hardcoded products by price (low-to-high) before rendering
            hardcodedProducts.sort((a, b) => a.price - b.price);
            renderProducts(hardcodedProducts); // Render hardcoded products if fetch fails
        }
    }
